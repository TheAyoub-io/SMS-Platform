...
import { ContactList, ContactListUpdate } from '../services/contactListApi';
...
const EditContactListModal: React.FC<EditContactListModalProps> = ({
  isOpen,
  onClose,
  contactList,
}) => {
  const updateContactListMutation = useUpdateContactList();
  const { data: contacts } = useContacts({}, { skip: 0, limit: 1000 });
  
  const [formData, setFormData] = useState<ContactListUpdate>({
    nom_liste: '',
    type_client: '',
    zone_geographique: '',
  });
  ...
  useEffect(() => {
    if (isOpen && contactList) {
      setFormData({
        nom_liste: contactList.nom_liste,
        type_client: contactList.type_client,
        zone_geographique: contactList.zone_geographique,
      });
      setErrors({});
    }
  }, [isOpen, contactList]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateContactListMutation.mutateAsync({
  ...
  const handleInputChange = (field: keyof ContactListUpdate, value: string) => {
        ...
        <div className="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
          ...
          <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {/* Warning about changing type/zone */}
              {(formData.type_client !== contactList.type_client || 
                formData.zone_geographique !== contactList.zone_geographique) && (
                <div className="bg-yellow-50 border border-yellow-200 rounded-md p-3">
                  <p className="text-sm text-yellow-800">
                    <strong>Warning:</strong> Changing the contact type or geographic zone will 
                    automatically update the contacts in this list to match the new criteria.
                  </p>
                </div>
              )}
              ...
              {formData.type_client && formData.zone_geographique && (
                <div className="bg-blue-50 border border-blue-200 rounded-md p-3">
                  <p className="text-sm text-blue-800">
                    <strong>Note:</strong> This list will include all contacts with type "{formData.type_client}" 
          ...
          <div className="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
            <button
              ...
              disabled={updateContactListMutation.isLoading}
              className="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-indigo-600 text-base font-medium text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:ml-3 sm:w-auto sm:text-sm disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {updateContactListMutation.isLoading ? 'Updating...' : 'Update List'}
            </button>
            <button
              type="button"
              onClick={onClose}
